...
\item[(d)] Block bound: if $C_2 \succ 0$, then
$\TT\big([P_1\ P_2], \diag(C_1, C_2)\big) \le \TT(P_1, C_1) + \op{\Sig}^{2}\,\Tr\!\big(P_2\, C_2^{-1} P_2^\top\big)$; in particular, for $C_2 = \diag(\lambda^{(j)})_j$ the second term is $\op{\Sig}^2\sum_j \lVert P_{2,j}\rVert^2/\lambda^{(j)}$.
...
\]
and $\mathrm{EV}(x;\zeta_1) = \TT(P_1, C_1)$. For diagonal $C_2$ the trace is $\sum_j\lVert P_{2,j}\rVert^2/\lambda^{(j)}$ with $P_{2,j}$ the $j$-th column.
...
\]
where in (d) the constant is sharp because $\Cov(r) \preceq \Sig$ gives $\op{\Cov(r,x)} \le \op{\Sig}$. Lemma~\ref{lem:calc}(a) converts this to the boxed inequality of the theorem. Nothing here constrains $k$, $d$, or the offsets, and $p_0$ enters only through $\Sig \succ 0$ and the exponential-moment condition \eqref{eq:expmom}.
