...
\]
where in (d) the constant is sharp because $\Cov(r) \preceq \Sig$ gives $\op{\Cov(r,x)} \le \op{\Sig}$. \emph{That last step is where the conditioning of $\Sig$ is discarded; Remark~\ref{rem:covr} records what is available if it is not taken.} Lemma~\ref{lem:calc}(a) converts this to the boxed inequality of the theorem. Nothing here constrains $k$, $d$, or the offsets, and $p_0$ enters only through $\Sig \succ 0$ and the exponential-moment condition \eqref{eq:expmom}.
