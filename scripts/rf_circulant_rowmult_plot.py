...
r=np.load('tables/rf_circulant_rowmult_A.npz'); z=np.load('tables/rf_circulant_avgpool_scaleup.npz')
...
for i,s in enumerate(SIG):
    ...
    for m,col,mk in zip(MS,('#e07b39','#c0392b','#7b3fa0'),('^','s','D')):
        ys=[float(r[f'{j}|{m}|{s}'][0]) for j in js if f'{j}|{m}|{s}' in r.files]
        es=[float(r[f'{j}|{m}|{s}'][1]) for j in js if f'{j}|{m}|{s}' in r.files]
        xs=[j for j in js if f'{j}|{m}|{s}' in r.files]
        ax.errorbar(xs,ys,yerr=es,color=col,marker=mk,lw=2.2,ms=5,capsize=3,
                    label=rf'$\mathcal{{L}}^{{\rm circ}}$, $m={m}$ ($c={m}j$, {m}$\times$rows, '
...
fig.savefig('figures/rf_circulant_rowmult.png',dpi=150,bbox_inches='tight')
print("Saved figures/rf_circulant_rowmult.png")
