...
rB=np.load('tables/rf_circulant_rowmult_A_t8_all.npz'); rF=np.load('tables/rf_circulant_rowmult_A.npz'); r=rB; z=np.load('tables/rf_circulant_avgpool_scaleup.npz')
zs={round(float(s),3):s for s in z['sigmas']}
JS=[1,2,4,8,16]; MS=[2,4,8,16,32]; d=512
...
for i,s in enumerate(SIG):
    ...
    for m,col,mk in zip(MS,('#f0a860','#e07b39','#c0392b','#7b3fa0','#2d7d46'),('^','s','D','v','P')):
        ...
        ax.plot(xf,yf,color=col,lw=1.2,ls=':',alpha=.75,
                label=(rf'full width (reference)' if m==2 else None)) if f'{js[0]}|{m}|{s}' in rF.files else None
...
fig.savefig('figures/rf_circulant_rowmult_banded_all.png',dpi=150,bbox_inches='tight')
print("Saved figures/rf_circulant_rowmult_banded_all.png")
