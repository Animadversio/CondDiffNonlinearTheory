...
sig = np.array(SIGS)
W, EDM, EDMSE, P2, B1, B2 = ({} for _ in range(6))
NS = {}
for s in SIGS:
    w = get(H, 'linear|{}', s)
    # The SAME Wiener row is stored in all three tables (linear_split has no RNG).  Verify
    # rather than assume -- a mismatch would mean the splits moved underneath one of them.
    for tag, z in (('band', B), ('edm', E)):
        o = get(z, 'linear|{}', s)
...
print(' sigma   W_test    EDM_te  (se)   2D c=3072   B=1 c=512   B=2 c=512   seeds(2D,B1,B2)')
for s in SIGS:
    print(f' {s:5.3f} {W[s]:8.4f}  {EDM[s]:8.4f} {EDMSE[s]:.4f} '
          f'{P2[s]:11.4f} {B1[s]:11.4f} {B2[s]:11.4f}       {NS[s]}')
print('\n excess over the HELD-OUT Wiener (negative = beats linear on the same 10k images):')
for s in SIGS:
    print(f' {s:5.3f}  EDM {EDM[s]-W[s]:+8.4f}   2D {P2[s]-W[s]:+8.4f}   '
          f'B=1 {B1[s]-W[s]:+8.4f}   B=2 {B2[s]-W[s]:+8.4f}')
...
for i, (lab, d, col, mk, ls, lw) in enumerate(series[1:]):
    ...
    short = {'plain 2-D RF  c=3072': 'plain 2-D c=3072',
             ...
             'band RF  c=512  B=2': 'band c=512 B=2'}[lab]
...
for s in SIGS:
    g = W[s] - EDM[s]
    print(f' {s:5.3f}  gap {g:6.3f}   2D {100*(W[s]-P2[s])/g:+7.1f}%   '
